...
int print(const char *name, const char *output,char *sF,int* compare,int compare_size);
		...
		if (entry->d_type == DT_DIR) {
			...
			if (child_pid >= 0) /* fork succeeded */
			{
				if (child_pid == 0) /* fork() returns 0 for the child process */
				{
					...
					process_count++;
					int temp = print(entry->d_name, output,sF,compare,compare_size);    // recursive call
					//printf("temp is :%d\n",temp);
					process_count += temp;
					//printf("func returned %d,%d\n",print(entry->d_name, output),process_count);
					fflush(stdout);
					_exit(process_count);
				} else {/* parent process */
					...
					if (child2_pid >= 0){ /* fork succeeded */
						if (child2_pid == 0){ /* fork() returns 0 for the child process */
							...
							/*printf("Child PID =  %d, Parent pid = %d ; filename: %s : process_count %d\n",
									getpid(), getppid(),entry->d_name,process_count);*/
							//printf("Child PID =  %d, Parent pid = %d\n", getpid(), getppid());
							printf("%d, ",getpid());
							...
							if (valid) {
								...
								file_sort(sF, fp, output,entry->d_name,compare,compare_size);
...
//--------
	char*cat[] = {"color","director_name","num_critic_for_reviews","duration","director_facebook_likes","actor_3_facebook_likes","actor_2_name","actor_1_facebook_likes"\
