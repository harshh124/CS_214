...
int isspace(int c);
int convert(int sF);
int file_sort(char* field, FILE *fp, const char * output_dir, char * filename,int* compareArr,int compareArr_size);
...
int compare(struct mData leftArr, struct mData rightArr, int comp_ptr)
{
	...
	switch(comp_ptr)
	{
		...
		break;
		case 2:
			return strcmp(leftArr.review,rightArr.review);
		break;
		case 3:
			return (leftArr.duration) - (rightArr.duration);
		break;
		case 4:
			return strcmp(leftArr.dFbLikes,rightArr.dFbLikes);
		break;
		case 5:
...
void mergeData(struct mData *array,int left , int middle , int right, int* comp_ptr,int comp_ptr_size) // Merges the two arrays together returns a combined array
{
	int size1,size2;
	int counter = 0;
	size1 = middle-left+1;
...
void split(struct mData *array, int left, int right,int* comp_ptr,int comp_ptr_size)
{
